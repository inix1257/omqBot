...
public final class Koohii {
    ...
    public static class Vector2
    {
        ...
        public static boolean compare(Vector2 pos1, Vector2 pos2){
    ...
    public static class Slider
    {
        ...
        public ArrayList<Vector2> sliderPoints = new ArrayList<>();

        public String sliderType = "B";

        public double sliderTime = 0.0d;

        public Path2D sliderPath;
    ...
    public static class Map
    {
        ...
        public void reset()
        {
            ...
            ncircles = nsliders = nspinners = previewTime = 0;
            hp = cs = od = ar = 5.0f;
            sv = tick_rate = 1.0f;
...
exception since you can check lastline/lastpos when you catch */
    ...
    public static class Parser
    {
        ...
        private void general()
        {
            ...
            else if (p[0].equals("PreviewTime")) {
                beatmap.previewTime = Integer.parseInt(setlastpos(p[1]));
            }
        }
        ...
        private void objects()
        {
            ...
            obj.pos.x = Integer.parseInt(setlastpos(s[0]));
            obj.pos.y = Integer.parseInt(setlastpos(s[1]));
            ...
            else if ((obj.type & OBJ_SLIDER) != 0)
            {
                ...
                sli.sliderPoints.add(new Vector2(sli.pos.x, sli.pos.y));
                sli.sliderType = s[5].split("\\|")[0];
                for(int i = 1 ; i < (s[5].split("\\|")).length; i ++){
                    String str = s[5].split("\\|")[i];
                    double posX = Double.parseDouble(str.split(":")[0]);
                    double posY = Double.parseDouble(str.split(":")[1]);
                    sli.sliderPoints.add(new Vector2(posX, posY));
                }
                ...
                for(int i = 0 ; i < beatmap.tpoints.size(); i ++){
                    Timing t = beatmap.tpoints.get(i);
                    if(t.time > obj.time) continue;
                    if(t.ms_per_beat >= 0) BPM = t.ms_per_beat;
                    else if(!t.change) speed = t.ms_per_beat;
                }

                sli.sliderTime = sli.distance / (beatmap.sv * 100 * (-100 / speed)) * BPM;
                sli.sliderTime *= sli.repetitions;
